...
";
  ...
  static function remove($groupID = NULL, $onceOnly = TRUE) {
    ...
    // DS: For llr there are quite a lot of deadlocks happening during cache
    // resets. We we making groupID as a required var with assumption that cache 
    // should forcibly be reset only when group-id is known.
    if ($groupID == NULL) {
      return;
    }
    if ($groupID == NULL) {
