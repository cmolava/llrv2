{literal}
  <script type="text/javascript">
    ...
    cj(document).ready(function(){
      ...
      cj("#contributionSearch").find("input[type='checkbox']").on("click",function(){
        
          var position          = cj(this).offset();
          var fixedLeft         = position.left - 200 ;
          ...
          if(cj(this).attr('id') == 'toggleSelect'){
            if(cj(this).prop("checked") == true){
              ...
              cj("#floating_total_amount").html("Selected Amount : " + currencySymbol +" "+totalAmount.toFixed(2));
          }
          ...
          if(cj(this).prop("checked") == true){
            ...
            var totalSelectedValue  = parseFloat(selectedValue[4]);
            var explodetotalAmount  = cj('#rowid'+splitId[2]+' td.crm-contribution-amount span').text().split(" ");
            var amount              = parseFloat(explodetotalAmount[1]);
            ...
            if(totalSelectedValue == 0){
              ...
              if(isNaN(totalSelectedValue)){
                cj("#floating_total_amount").html("Selected Amount : " + currencySymbol +" "+amount.toFixed(2));
              }else{
                cj("#floating_total_amount").html("Selected Amount : " + currencySymbol +" "+totalAmount.toFixed(2));
              }
            }
          ...
          if(cj(this).prop("checked") == false){
            ...
            var totalminusResult    = parseFloat(minusResult[4]);
            var removeRow           = parseFloat(totalminusResult) - parseFloat(amount);
            if(isNaN(removeRow) || removeRow == 0){
              cj('#floating_total_amount').hide();
              cj("#floating_total_amount").empty();
            }else{
              cj("#floating_total_amount").html("Selected Amount : " + currencySymbol +" "+removeRow.toFixed(2));
            }
          }
        }
      });
