...
class CRM_Utils_ReminderActivityViaJob {
  ...
  function __construct($jobID, $entity = 'civicrm_activity') {
    $query = "
      ...
      INNER JOIN civicrm_action_mapping cam  ON cas.mapping_id = cam.id AND cam.entity = %2
      WHERE      cmj.id = %1";

    $actionSchedule = CRM_Core_DAO::executeQuery($query, array(1 => array($jobID, 'Integer'), 2 => array($entity, 'String')));
...
{$whereClause}
  ...
  function buildParticipantInfoQuery($cids = array()) {
    $query = "
        SELECT cp.contact_id        as contactID, 
               eve.id               as event_id,
               eve.title            as event_title,
               parti.first_name     as first_name,
               parti.last_name      as last_name,
               regby.first_name     as regby_first_name,
               regby.last_name      as regby_last_name
          FROM civicrm_participant  cp
    INNER JOIN civicrm_event        eve       ON cp.event_id = eve.id
    INNER JOIN civicrm_contact      parti     ON cp.contact_id = parti.id
     LEFT JOIN civicrm_participant  regbyp    ON cp.registered_by_id = regbyp.id
     LEFT JOIN civicrm_contact      regby     ON regbyp.contact_id = regby.id
     WHERE cp.contact_id IN (" . implode(',', $cids) . ") AND cp.event_id IN (" . implode(',', $this->_value) . ")";
