...
class CRM_Contact_Form_Search_Custom_FMNFContributions
   implements CRM_Contact_Form_Search_Interface {
    ...
    function buildForm( &$form ) {
         ...
         */
        $this->setTitle('Find Contributors using day range for LAST contribution');

        $form->addElement('checkbox', 'fmnf_contribs', ts( 'FMNF Contributions Only' ) );

        $form->addElement('checkbox', 'tf_contribs', ts( 'Tribute Fund Contributions Only' ) );
        
        $form->addElement('checkbox', 'hon_contribs', ts( 'In Honor of Contributions Only' ) );

        $form->addElement('checkbox', 'mem_contribs', ts( 'In Memory of Contributions Only' ) );
        ...
        $form->add( 'text',
                    'min_days',
                    ts( 'From (LAST Contribution number of days)' ) );
...
";
    ...
    function where( $includeContactIDs = false ) {
        ...
        if ( $honContribs ) {        
            $clauses[] = "contrib.honor_type_id = 1 ";
        }

        $memContribs = CRM_Utils_Array::value( 'mem_contribs',
                                          $this->_formValues );
        
        if ( $memContribs ) {        
            $clauses[] = "contrib.honor_type_id = 2 ";
        }
        ...
        $FMNFContribs = CRM_Utils_Array::value( 'fmnf_contribs',
                                          $this->_formValues );
        if ( $FMNFContribs ) {
            $clauses[] = "contact_fmnf.id = contrib.honor_contact_id and contact_fmnf.contact_type = 'Individual' and contact_fmnf.contact_sub_type = 'Forget_Me_Not_Fund' ";
        }
                  
        $TFContribs = CRM_Utils_Array::value( 'tf_contribs',
                                          $this->_formValues );
        if ( $TFContribs ) {
            $clauses[] = "contact_c.id = contrib.honor_contact_id and contact_b.contact_type = 'Individual' and contact_b.contact_sub_type = 'Tribute_Fund' ";
        }
        ...
        if ( $minDays ) {
            $clauses[] = "DATEDIFF(CURDATE(),contrib.receive_date) >= $minDays";
        }

        $maxDays = CRM_Utils_Array::value( 'max_days', $this->_formValues );
        if ( $maxDays ) {
            $clauses[] = "DATEDIFF(CURDATE(),contrib.receive_date) <= $maxDays";
        }
    ...
    function having( $includeContactIDs = false ) {
        ...
        $minDays = CRM_Utils_Array::value( 'min_days', $this->_formValues  );
        if ( $minDays ) {
            $clauses[] = "DATEDIFF(CURDATE(),MIN(contrib.receive_date)) >= $minDays";
        }

        /**
         * PS 22/01/2012 Commented out as FMNF contributions should only be using the
         * Minimum number of days for the first contribution
         * The other rule is that at least one contribution must have been made between the start and end dates
         * Which is taken care of in the main where
        $maxDays = CRM_Utils_Array::value( 'max_days', $this->_formValues );
        if ( $maxDays ) {
            $clauses[] = "DATEDIFF(CURDATE(),MIN(contrib.receive_date)) <= $maxDays";
        }
         * PS 22/01/2012 End of Commented out section
         */
