...
function dfptokens_civicrm_tokens(&$tokens) {
  ...
  $tokens['uk_co_vedaconsulting_participant'] = array(
    'uk_co_vedaconsulting_participant.claim_url'  => 'DFP: Participant: Claim URL',
    'uk_co_vedaconsulting_participant.first_name' => 'DFP: Participant: First Name',
    'uk_co_vedaconsulting_participant.last_name'  => 'DFP: Participant: Last Name',
    'uk_co_vedaconsulting_participant.regby_first_name' => 'DFP: Participant: Registered By: First Name',
    'uk_co_vedaconsulting_participant.regby_last_name'  => 'DFP: Participant: Registered By: Last Name',
    'uk_co_vedaconsulting_participant.event_id'      => 'DFP: Participant: Event ID',
    'uk_co_vedaconsulting_participant.event_title'   => 'DFP: Participant: Event Title',
  );
}
...
function dfptokens_civicrm_tokenValues(&$values, $cids, $job = null, $tokens = array(), $context = null) {
  ...
  if (array_key_exists('uk_co_vedaconsulting_participant', $tokens) && $job) {
    $remActObj = new CRM_Utils_ReminderActivityViaJob($job, 'civicrm_participant');
    if ($remActObj->_actionScheduleId) {
      $query = $remActObj->buildParticipantInfoQuery($cids);
      $data  = CRM_Core_DAO::executeQuery($query);
      while ($data->fetch()) {
        $values[$data->contactID]['uk_co_vedaconsulting_participant.first_name']  = $data->first_name;
        $values[$data->contactID]['uk_co_vedaconsulting_participant.last_name']   = $data->last_name;
        $values[$data->contactID]['uk_co_vedaconsulting_participant.event_id']    = $data->event_id;
        $values[$data->contactID]['uk_co_vedaconsulting_participant.event_title'] = $data->event_title;
        $values[$data->contactID]['uk_co_vedaconsulting_participant.regby_first_name'] = $data->regby_first_name;
        $values[$data->contactID]['uk_co_vedaconsulting_participant.regby_last_name']  = $data->regby_last_name;
        
        $uid = CRM_Core_BAO_UFMatch::getUFId($data->contactID);
        if ($uid && module_load_include('module', 'llr_event', 'llr_event')) {
          $url = llr_event_login_dfp_form_url($uid, $data->event_id);
          $values[$data->contactID]['uk_co_vedaconsulting_participant.claim_url'] = CRM_Utils_System::url($url, NULL, TRUE);
        }
      }
    }
  }
