...
class CRM_Core_BAO_File extends CRM_Core_DAO_File {
   ...
   */
  public static function deleteFileReferences($fileID, $entityID, $fieldID, $isDedupeMode = FALSE) {
    $fileDAO = new CRM_Core_DAO_File();
    $fileDAO->id = $fileID;
    if (!$fileDAO->find(TRUE)) {
    ...
    if ($entityFileDAO->find(TRUE)) {
      ...
      $fileDAO->delete();
    } else if (!$isDedupeMode) { 
      // For dedupe we skip fatal, and continue with updating file.
      CRM_Core_Error::fatal();
    }
