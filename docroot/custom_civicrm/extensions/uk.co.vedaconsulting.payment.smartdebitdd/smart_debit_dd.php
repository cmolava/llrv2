...
class uk_co_vedaconsulting_payment_smartdebitdd extends CRM_Core_Payment {
  ...
  function preparePostArray( $fields, $self = null ) {
    ...
    if ( isset( $fields['contactID'] ) ) {
      $extID = CRM_Core_DAO::getFieldValue('CRM_Contact_DAO_Contact', $fields['contactID'], 'external_identifier');
      $payerReference = $extID ? $extID : $fields['contactID'];
...
//    $validateParams['bank_account_number'] = null;
    ...
    switch ( strtoupper( $response["Status"] ) ) {
        case 'OK':
            ...
            // set the session var to ok, incase has been set as failed with
            // earlier validation attempts
            $_SESSION['contribution_attempt'] = 'ok';
...
*/
  ...
  private function succeed( $response, &$params ) {
    ...
    UK_Direct_Debit_Form_Main::createDirectTransferRecord($response['trxn_id'], $params['contactID'], $response['start_date'], $params['campaign_id'], $params['contributionRecurID'], $params['amount']);

    // specific to llr. Generates DD signup pdf which will be attached with
    // notification mail at the end 
    UK_Direct_Debit_Form_Main::createDDSignupPdf($response, $params);
...
*/
  ...
  function buildForm( &$form ) {
    ...
    if (self::getCRMVersion() >= 4.2) {
      //CM:Template doesn't exist, temp fix to avoid smarty error
