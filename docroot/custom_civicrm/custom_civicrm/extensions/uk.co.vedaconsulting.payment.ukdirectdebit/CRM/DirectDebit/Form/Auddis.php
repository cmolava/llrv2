...
class CRM_DirectDebit_Form_Auddis extends CRM_Core_Form {
  ...
  function buildQuickForm() {
    ...
    if(!empty($missingTrxnIds)) {
      ...
      $findMissingQuery = "
          SELECT `transaction_id` as trxn_id, contact as display_name, amount as amount, receive_date as receive_date
      ...
      while($dao->fetch()) {
        ...
        // If contact found from the above hook, add that contact to matched contact's list
        if(!empty($params['contact_id'])) {
          $receiveDate = date('Y-m-d', strtotime(str_replace('/', '-', $dao->receive_date)));
          $contTrxnId  = $params['transaction_id'].'/'.CRM_Utils_Date::processDate($receiveDate);
          $findContrQuery   = "SELECT id from civicrm_contribution where contact_id = %1 and trxn_id = %2";
          
          $findParams = array(
            1 => array($params['contact_id'], 'Int'),
            2 => array($contTrxnId, 'String'),
          );
          if(!CRM_Core_DAO::singleValueQuery($findContrQuery, $findParams)) {
            array_push($listArray, $params);
            array_push($matchTrxnIds , "'".$dao->trxn_id."' ");
          }
        }
        else {
          $missingArray[$key] = $params;
          $key++;
        }
