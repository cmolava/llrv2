...
class CRM_Contact_Form_Search_Custom_ActivityDaysSearch
implements CRM_Contact_Form_Search_Interface {
    ...
    function buildForm( &$form ) {
         ...
         */
        $this->setTitle('Find Contacts who have Activities within certain number of days');
        ...
        $form->assign( 'elements', array( 'contact_type', 'activity_subject', 'activity_type_id', 
                                          'activity_status_id', 'min_days', 'max_days', 'start_date', 'end_date', 'sort_name') );
    }
    ...
    function all( $offset = 0, $rowcount = 0, $sort = null,
        ...
        if ( $onlyIDs ) {
            $select  = 'IFNULL(contact_d.id, contact_a.id) as contact_id'; 
        } else {
            $select  = '
                IFNULL(contact_d.id, contact_a.id)      as contact_id,
                IFNULL(contact_d.sort_name, contact_a.sort_name)      as sort_name,
                IFNULL(contact_d.contact_type, contact_a.contact_type)      as contact_type,
    ...
    function from( ) {
        ...
        civicrm_contact contact_a
            ...
            LEFT JOIN civicrm_contact contact_d 
                 ON activity_target.target_contact_id = contact_d.id ";
    }
    ...
    function where( $includeContactIDs = false ) {
        ...
        if (! empty($this->_formValues['contact_type']) ) {
            $clauses[] = "IFNULL(contact_d.contact_type , contact_a.contact_type) LIKE '%{$this->_formValues['contact_type']}%'";
        }
        ...
        if ( $minDays ) {
            $clauses[] = "DATEDIFF(CURDATE(),activity.activity_date_time) >= $minDays";
        }

        $maxDays = CRM_Utils_Array::value( 'max_days', $this->_formValues );
        if ( $maxDays ) {
            $clauses[] = "DATEDIFF(CURDATE(),activity.activity_date_time) <= $maxDays";
        }
