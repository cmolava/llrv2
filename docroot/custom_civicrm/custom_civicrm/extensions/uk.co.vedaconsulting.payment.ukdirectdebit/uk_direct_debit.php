...
//            $form->assign( 'company_address', UK_Direct_Debit_Form_Main::getCompanyAddress());
        ...
        // make state province not required for donation signup form 
        // e.g contrib page id=5
        $bltID = $form->_bltID;
        foreach ($form->_paymentFields as $name => $field) {
          if (!empty($field['cc_field'])) {
            if ($field['htmlType'] == 'chainSelect' && $field['name'] == "billing_state_province_id-{$bltID}" ) {
              $form->_paymentFields[$name]["is_required"] = FALSE;
              break;
            }
          }
        }
...
function uk_direct_debit_civicrm_navigationMenu( &$params ) {
    ...
    $params[$parentId]['child'][$smartDebitId]= array(
                            'attributes' => array (
                                              'label'      => 'Import Smart Debit Contributions',
